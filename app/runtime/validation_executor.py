...
def _normalized_steps(plan: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in raw_steps:
        ...
        if not command:
            raise ValueError("Validation step command is required")
...
def _pytest_args(command: str) -> list[str]:
    args = [part for part in shlex.split(command, posix=False) if part]
    if not args:
        raise ValueError("Validation step command cannot be empty")
    return args
...
def run_validation_plan(plan: Mapping[str, Any]) -> ValidationExecutionResult:
    ...
    for step in _normalized_steps(plan):
        ...
        exit_code = pytest.main(_pytest_args(str(step["command"])))
        ...
        if not passed:
            step_result["failure_summary"] = f"pytest exit code {int(exit_code)}"
