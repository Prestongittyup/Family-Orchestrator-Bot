...
def _index_signals(signals: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {signal["id"]: signal for signal in signals}


def _index_correlations(
    correlations: list[dict[str, Any]],
) -> tuple[dict[str, str], dict[str, set[str]]]:
    signal_to_correlation: dict[str, str] = {}
    correlation_to_signals: dict[str, set[str]] = {}

    for correlation in correlations:
        correlation_id = correlation["correlation_id"]
        ids = set(correlation.get("signal_ids", []))
        correlation_to_signals[correlation_id] = ids
        for signal_id in ids:
            signal_to_correlation[signal_id] = correlation_id

    return signal_to_correlation, correlation_to_signals
...
def _build_gate_dag(
    ...
    node_id = f"decision-{proposal['id']}"
    operation = str(proposal.get("type", "task_action"))

    return DAG(
        dag_id=f"dag-{proposal['id']}",
        intent_id=f"intent-{proposal['id']}",
        nodes={
            node_id: DAGNode(
                node_id=node_id,
                node_type="task",
        ...
        entry_node=node_id,
        exit_nodes=[node_id],
        metadata={"user_id": user_id, "household_id": household_id},
    )


def _get_linked_signals(
    proposal: dict[str, Any],
    ...
    direct = [signal for signal in signals if signal.get("source_module") == source_module]

    linked_ids: set[str] = {signal["id"] for signal in direct}
    for signal in direct:
        correlation_id = signal_to_correlation.get(signal["id"])
        if correlation_id:
            linked_ids.update(correlation_to_signals.get(correlation_id, set()))

    return [signal_by_id[signal_id] for signal_id in sorted(linked_ids) if signal_id in signal_by_id]
...
def _classify_decision_type(
    ...
    normalized_priority = float(proposal.get("normalized_priority", 0.0))

    has_high_signal = any(signal.get("severity") == "high" for signal in linked_signals)
    has_medium_signal = any(signal.get("severity") == "medium" for signal in linked_signals)
    has_correlated_signal = any("correlation_id" in signal for signal in linked_signals)
    has_schedule_conflict = any(signal.get("type") == "schedule_conflict" for signal in linked_signals)

    if normalized_priority >= 7.5 or has_high_signal or has_schedule_conflict:
        return "interrupt", "high_priority_or_conflict", 0.95

    if 4.0 <= normalized_priority <= 7.49 or has_medium_signal:
        return "suggestion", "medium_priority_or_signal", 0.8

    if normalized_priority < 4.0 and not has_correlated_signal:
        return "notification", "low_priority_no_correlation", 0.65

    return "suggestion", "default_semantic_fallback", 0.75


def activate_decision_layer(
    ...
    *,
    ...
    proposals = list(orchestrator_output.get("proposals", []))
    signals = list(orchestrator_output.get("signals", []))
    semantic = dict(orchestrator_output.get("semantic_layer", {}))

    ordering_index = list(semantic.get("ordering_index", []))
    correlations = list(semantic.get("signal_correlations", []))

    signal_by_id = _index_signals(signals)
    signal_to_correlation, correlation_to_signals = _index_correlations(correlations)

    ordered_ids = [entry["proposal_id"] for entry in ordering_index]
    proposal_by_id = {proposal["id"]: proposal for proposal in proposals}

    safety_gate = gate or ExecutionGate()
    ...
    for proposal_id in ordered_ids:
        ...
        linked_signals = _get_linked_signals(
            proposal,
            signals,
            signal_to_correlation,
            correlation_to_signals,
            signal_by_id,
        )

        decision_type, reason, confidence = _classify_decision_type(proposal, linked_signals)

        gate_dag = _build_gate_dag(proposal, user_id=user_id, household_id=household_id)
        gate_decision = safety_gate.evaluate(
            dag=gate_dag,
            user_id=user_id,
        ...
        if gate_decision.status == ExecutionStatus.REJECT:
            decision_type = "notification"
            reason = "safety_gate_downgrade"
            confidence = 0.55
    ...
    return {
        "proposals": proposals,
        "signals": signals,
        "by_module": orchestrator_output.get("by_module"),
        "metadata": dict(orchestrator_output.get("metadata", {})),
...
# Backward-compatible v2 engine implementation migrated from decision_engine_v2.py
