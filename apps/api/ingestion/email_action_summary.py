...
from archive.apps.api.ingestion.email_priority_llm import maybe_refine_email_priority
...
_RULE_IGNORE_SENDERS = _sender_policy_set("EMAIL_RULE_IGNORE_SENDERS")
...
def _base_score(features: EmailFeatures) -> int:
    score = 0
    if _sender_matches_policy(features.normalized_sender, _ALWAYS_HIGH_SENDERS):
        score += 10
    elif _term_hits(features.normalized_sender, _SENSITIVE_SENDERS) > 0 and not _sender_is_junk_domain(features.normalized_sender):
    ...
    if _sender_matches_policy(features.normalized_sender, _ALWAYS_LOW_SENDERS):
        score -= 6
    if _sender_matches_policy(features.normalized_sender, _RULE_IGNORE_SENDERS):
        score -= 8

    return score


def _thread_state_body(*, body: str, thread_messages: list[Any] | None) -> str:
    if not thread_messages:
        return body

    snippets: list[str] = []
    for item in thread_messages[-3:]:
        if isinstance(item, dict):
            text = str(item.get("body") or item.get("snippet") or item.get("text") or "")
        else:
            text = str(item)
...
def _has_hard_negative_markers(*, sender: str, subject: str, body: str) -> bool:
    normalized_sender = _normalize_text(sender)
    normalized_subject = _normalize_text(subject)
    normalized_body = _normalize_text(body)
    combined = f"{normalized_subject} {normalized_body}".strip()

    if _sender_matches_policy(normalized_sender, _RULE_IGNORE_SENDERS):
        return True
    if _sender_matches_policy(normalized_sender, _ALWAYS_LOW_SENDERS):
        return True

    if "unsubscribe" in combined:
        return True
    if "newsletter" in normalized_subject:
        return True

    junk_hits = _term_hits(combined, _JUNK_TERMS)
    business_hits = _term_hits(combined, _BUSINESS_SIGNAL_TERMS)
    if junk_hits >= 3 and business_hits == 0:
        return True

    return False


def _priority_to_importance(priority: str) -> tuple[float, str]:
...
def _normalize_due_date_only(value: Any) -> str | None:
    if value is None:
        return None
    ...
    *,
    ...
    if features.contains_question:
        actions.append(
            {
                "type": "reply",
                "title": f"Reply re: {_clean_subject(features.subject)}",
                "urgency": "high" if priority == "high" else "normal",
                "due": None,
            }
        )

    for row in action_items[:3]:
        title = str(row.get("title") or "").strip()
        if not title:
            continue

        due = _normalize_due_date_only(row.get("due_hint"))
        actions.append(
            {
                "type": "task",
                "title": title,
                "urgency": "high" if float(row.get("importance_score") or 0.0) >= 0.78 else "normal",
                "due": due,
            }
        )
    ...
    for action in actions:
        key = (str(action.get("type")), str(action.get("title", "")).strip().lower())
        if key in seen:
            continue
        seen.add(key)
...
def _build_reason(*, features: EmailFeatures, is_junk: bool, rule_score: int) -> str:
    if is_junk:
        return "Promotional patterns or ignore policy triggered hard-negative handling."

    reasons: list[str] = []
    if features.contains_deadline:
        reasons.append("deadline language detected")
    if features.contains_action:
        reasons.append("action request detected")
    if features.contains_question:
        reasons.append("direct response requested")
    if features.is_direct:
        reasons.append("sent directly to recipient")
    if features.has_attachments:
        reasons.append("attachments included")

    if not reasons:
        reasons.append("conservative rule-based classification")

    return f"Rule score {rule_score}: " + ", ".join(reasons[:3]) + "."
...
def summarize_email_to_actions(
    *,
    ...
    thread_state_body = _thread_state_body(body=body, thread_messages=thread_messages)
    resolved_thread_length = thread_length if thread_length is not None else (len(thread_messages) if thread_messages else None)

    features = _build_features(
        sender=sender,
        subject=subject,
        body=thread_state_body,
    ...
    hard_drop = _has_hard_negative_markers(sender=sender, subject=subject, body=thread_state_body)
    if hard_drop:
        state_summary = "Promotional or ignored sender content was dropped before action extraction."
        reason = "Hard-negative prefilter matched sender policy or promotional unsubscribe signals."
        return {
    ...
    candidate_lines = _candidate_lines(subject=subject, body=thread_state_body)
    ...
    junk_score = _junk_score(sender=sender, subject=subject, body=thread_state_body)
    ...
    llm_refinement = maybe_refine_email_priority(
        sender=sender,
        subject=subject,
        body=thread_state_body,
        score=base_score,
        to_me=features.is_direct,
        cc_me=features.is_cc,
        thread_id=thread_id,
        latest_message_id=latest_message_id,
        thread_context=thread_state_body,
    )

    priority = priority_label.lower()
    needs_attention = priority == "high" or bool(action_items) or bool(calendar_candidates)
    ...
    llm_triage_decision = ""
    called_llm = llm_refinement is not None
    if llm_refinement is not None:
        refined_priority = str(llm_refinement.get("priority") or llm_refinement.get("priority_label") or "").strip().lower()
        if refined_priority in {"high", "medium", "low"}:
            priority = refined_priority
            priority_label = refined_priority.upper()

        refined_bucket = str(llm_refinement.get("importance_bucket") or "").strip().lower()
        if refined_bucket in {"critical", "high", "medium", "low"}:
            importance_bucket = refined_bucket

        refined_score = llm_refinement.get("importance_score")
        try:
            if refined_score is not None:
                importance_score = max(0.0, min(1.0, float(refined_score)))
        except (TypeError, ValueError):
            pass

        if "needs_attention" in llm_refinement:
            needs_attention = bool(llm_refinement.get("needs_attention"))

        raw_actions = llm_refinement.get("actions")
        if isinstance(raw_actions, list):
            ...
            for row in raw_actions:
                ...
                due = _normalize_due_date_only(row.get("due"))
            ...
            if normalized_actions:
                actions = normalized_actions[:4]

        llm_state_summary = str(llm_refinement.get("state_summary") or "").strip()
        llm_reason = str(llm_refinement.get("reason") or "").strip()

        if llm_refinement.get("is_junk") is True:
            is_junk = True

        llm_triage_decision = str(llm_refinement.get("triage_decision") or "").strip().lower()
    else:
        llm_state_summary = ""
        llm_reason = ""

    if _has_hard_negative_markers(sender=sender, subject=subject, body=thread_state_body):
        priority = "low"
        priority_label = "LOW"
        needs_attention = False
        actions = []
        is_junk = True
        llm_triage_decision = "junk"
    ...
    if llm_triage_decision == "junk":
        is_junk = True
        triage_decision = "junk"
    elif llm_triage_decision == "task":
        triage_decision = "task"
    elif llm_triage_decision == "informational":
        if priority_label != "HIGH" and not action_items and not calendar_candidates:
    ...
    mapped_importance_score, mapped_importance_bucket = _priority_to_importance(priority)
    if called_llm:
        importance_score = mapped_importance_score
        importance_bucket = mapped_importance_bucket

    if actions and not action_items:
        action_items = [
            {
                "title": str(action.get("title") or "").strip(),
                "details": "llm_action",
                "importance_score": 0.8 if str(action.get("urgency")) == "high" else 0.6,
                "importance_bucket": "high" if str(action.get("urgency")) == "high" else "medium",
                "due_hint": action.get("due"),
                "due_hint_local": _format_local_hint(action.get("due")),
                "source_line": 0,
            }
            for action in actions
    ...
    summary = llm_state_summary or _build_summary(
        features=features,
    ...
    reason = llm_reason or _build_reason(features=features, is_junk=is_junk, rule_score=rule_score)
