...
class EmailInput(BaseModel):
    ...
    provider: str = Field(
        default="generic",
        description="Email provider (for example 'google', 'outlook', 'yahoo', 'imap', or custom provider ids)",
    )
    thread_id: str | None = Field(
        default=None,
        description="Provider thread or conversation identifier",
    )
    latest_message_id: str | None = Field(
        default=None,
        description="Most recent message identifier for the thread snapshot",
    )
    thread_messages: list[Any] = Field(
        default_factory=list,
        description="Latest thread-state snippets used for thread-first triage",
    )
    to_me: bool | None = Field(
        default=None,
        description="Whether the message is directly addressed to the household inbox",
    )
    cc_me: bool | None = Field(
        default=None,
        description="Whether the message is in cc for the household inbox",
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "latest_message_id": "msg-abc123@gmail.com",
                "thread_messages": [
                    "Previous message context",
                    "Latest response in thread",
                ],
                "to_me": True,
                "cc_me": False,
            }
