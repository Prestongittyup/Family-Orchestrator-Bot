...
from archive.apps.api.ingestion.adapters.email_provider_adapter import EmailProviderAdapter
...
def _ingest_parsed_message(parsed: dict[str, Any]) -> dict[str, Any]:
    try:
        result = ingest_email(
            ...
            thread_id=str(parsed.get("thread_id") or "").strip() or None,
            latest_message_id=str(parsed.get("latest_message_id") or parsed["email_id"]),
            thread_messages=list(parsed.get("thread_messages") or []),
            to_me=parsed.get("to_me"),
            cc_me=parsed.get("cc_me"),
        )
        return {
            "status": "processed",
            "result": result,
