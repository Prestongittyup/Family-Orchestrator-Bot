...
def _validate_health_contract() -> tuple[bool, str]:
    app = create_app()
    route_paths = {
        getattr(route, "path", "")
    ...
    if "/healthz" not in route_paths:
        return False, "Missing canonical health endpoint: /healthz"

    noncanonical_health_routes = sorted(
        path
        for path in route_paths
        if path != "/healthz" and ("health" in path or "ready" in path or ("boot" in path and "system" in path))
    )
    if noncanonical_health_routes:
        return False, f"Non-canonical health routes still registered: {noncanonical_health_routes}"

    return True, "Single health contract validated (/healthz only)"
...
if __name__ == "__main__":
    ...
    print("\n[STAGE 2] Health Contract Validation")
    health_ok, health_message = _validate_health_contract()
    if not health_ok:
        print(f"[FAIL] Health contract: {health_message}")
        ...
        report = {
            ...
            "failure_attribution": {
                "primary_cause": f"Health contract validation failed: {health_message}",
                "secondary_causes": [],
                "confidence": 1.0,
            },
            "determinism": {
    ...
    print("[PASS] Health contract validated")
