...
def approve_assistant_action(payload: AssistantApproveRequest, request: Request) -> AssistantApproveResponse:
    ...
    request_id = str(payload.request_id or action_payload.get("request_id", ""))
    if not request_id:
...
def _infer_household_id_for_action(*, action_id: str, request_id: str | None, household_id: str) -> str:
    ...
    try:
        payload = runtime_orchestrator.state_store._read_store()  # type: ignore[attr-defined]
    except Exception:
        return household_id

    households = payload.get("households", {})
    for candidate_household_id, graph in households.items():
        actions = graph.get("action_lifecycle", {}).get("actions", {})
        responses = graph.get("responses", {})
