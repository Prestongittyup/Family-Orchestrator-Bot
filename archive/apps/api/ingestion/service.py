...
from archive.apps.api.core.feature_flags import resolve_feature_flags
from archive.apps.api.identity.repository import IdentityRepository
from archive.apps.api.ingestion.failure_handling import build_failure_trace, quarantine_ingestion_payload
from archive.apps.api.ingestion.email_action_summary import summarize_email_to_actions
from archive.apps.api.ingestion.models import (
...
from archive.apps.api.ingestion.normalization import (
...
from archive.apps.api.observability.trace_context import ensure_event_payload_trace
...
def _priority_from_importance_bucket(bucket: str) -> str:
...
def _first_dict_result(result: object) -> dict[str, Any] | None:
...
def _extract_email_processing_fields(result: object) -> dict[str, Any]:
    outcome = _first_dict_result(result)
    if outcome is None:
        return {}
    ...
    if status is not None:
        fields["processing_status"] = str(status)

    for key in ("task_id", "task_title", "calendar_event_id", "priority"):
...
def _resolve_household_id_for_email(
    *,
    recipient: str,
    explicit_household_id: str | None,
) -> str:
    normalized_explicit = (explicit_household_id or "").strip()
    ...
    if normalized_recipient:
        try:
            ...
            user = repo.get_user_by_email(normalized_recipient)
            if user is not None and getattr(user, "household_id", None):
                return str(user.household_id)
        except Exception:
            pass

    return get_household_for_source("email_ingestion")
...
def ingest_email(
    ...
    household_id_for_events = get_household_for_source("email_ingestion")
    try:
        # 1) Validate email payload at ingestion boundary
        email = validate_email_payload(payload)

        resolved_household_id = _resolve_household_id_for_email(
            recipient=str(email.recipient),
            explicit_household_id=(str(payload.get("household_id") or "").strip() or None),
        )
        ...
        analysis = summarize_email_to_actions(
        ...
        if is_junk:
            router.emit(
                SystemEvent.EmailParseFailed(
                    ...
                    reason="junk_filtered",
                    error_message="Email classified as junk and excluded",
                    raw_input={
                        **payload,
            ...
            response = {
                "status": "ignored_junk",
                "event_id": os1_event["idempotency_key"],
                "message": "Email classified as junk and ignored",
                "analysis": analysis,
            }
            debug = _debug_payload(
                flags.debug_mode,
        ...
        processing_fields = _extract_email_processing_fields(result)
