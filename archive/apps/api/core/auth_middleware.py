...
from archive.apps.api.auth.token_service import AuthValidationSystemError, TokenService
from archive.apps.api.identity.sqlalchemy_repository import SQLAlchemyIdentityRepository
from archive.apps.api.observability.logging import log_error, log_event
...
def _extract_request_token(request: Request) -> tuple[str | None, bool]:
    ...
    if request.url.path in _SSE_QUERY_TOKEN_PATHS:
        query_token = request.query_params.get("session_token") or request.query_params.get("access_token")
        if query_token:
            return query_token.strip(), had_bearer_header

    return None, had_bearer_header
