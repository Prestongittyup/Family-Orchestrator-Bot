...
class IdentityRepository(ABC):
    ...
    def __new__(cls, *args, **kwargs):
        # Backward compatibility: older callers instantiate IdentityRepository
        # directly with an optional session. Route those calls to the default
        # SQLAlchemy implementation while keeping this class as the abstract
        # interface for type contracts.
        if cls is IdentityRepository:
            from archive.apps.api.identity.sqlalchemy_repository import SQLAlchemyIdentityRepository

            return SQLAlchemyIdentityRepository(*args, **kwargs)
