...
from archive.apps.api.llm.provider import LLMIntentResponse, LLMProvider, build_llm_provider
...
def _coerce_intent_type(raw: str | None) -> IntentType | None:
    ...
    # LLM outputs usually match enum values (e.g., "create_task").
    try:
        return IntentType(normalized.lower())
    except ValueError:
        pass

    # Defensive fallback for enum-name style outputs (e.g., "CREATE_TASK").
    try:
        return IntentType[normalized.upper()]
    except KeyError:
...
def _merge_extracted(llm_extracted: dict) -> ExtractedFields:
    ...
    return ExtractedFields(
        data={k: v for k, v in (llm_extracted or {}).items() if v is not None}
    )
