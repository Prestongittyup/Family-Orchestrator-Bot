...
from archive.apps.assistant_core.contracts import (
    AlternativeRecord,
    AssistantIntent,
    AssistantPlan,
    AssistantResponse,
...
from archive.apps.assistant_core.meal_planner import default_inventory, default_recipe_history, plan_meal
...
def _load_artifact_summary() -> list[str]:
    ...
    for filename, label in artifact_map.items():
        path = artifact_read_path(filename)
        if not path.exists():
            continue
        try:
...
def _find_available_windows(events: list[dict[str, str]], reference_time: datetime) -> list[tuple[str, str, str]]:
    timezone = reference_time.tzinfo or UTC
    windows: list[tuple[str, str, str]] = []
    for offset in range(0, 7):
        day = reference_time.date() + timedelta(days=offset)
        for start_hour in (6, 7, 12, 16, 18):
            start_dt = datetime.combine(day, time(start_hour, 0), tzinfo=timezone)
...
def _build_meal_plan(
...
) -> tuple[AssistantPlan, list[ConflictRecord], list[AlternativeRecord], list[ProposedAction], list[str]]:
    meal = plan_meal(
    ...
    timezone = reference_time.tzinfo or UTC
    meal_start = datetime.combine(reference_time.date(), time(18, 30), tzinfo=timezone)
    meal_end = meal_start + timedelta(minutes=45)
    meal_window = _format_time_block(meal_start, meal_end)

    meal_block = TimelineBlock(
        time_block=meal_window,
    ...
    def build_response(
        ...
        *,
        ...
        if intent.intent_type == "appointment":
            ...
            plan, conflicts, alternatives, proposed_actions, reasoning = _build_meal_plan(
                intent,
                repeat_window_days,
                reference_time,
            )
