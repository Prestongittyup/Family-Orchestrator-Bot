...
@pytest.mark.system
@pytest.mark.legacy
def test_bootstrap_rolls_up_low_priority_email_notifications(monkeypatch) -> None:
    class _EventLogRow:
        def __init__(self, *, email_id: str, subject: str, priority: str, bucket: str) -> None:
            self.id = f"event-{email_id}"
            self.payload = {
                "email_id": email_id,
                "parsed_fields": {
                    "subject": subject,
                    "state_summary": f"Summary for {subject}",
                    "priority": priority,
    ...
    rows = [
        _EventLogRow(email_id="mail-high-1", subject="School reminder", priority="high", bucket="high"),
        _EventLogRow(email_id="mail-low-1", subject="Newsletter one", priority="low", bucket="low"),
        _EventLogRow(email_id="mail-low-2", subject="Newsletter two", priority="low", bucket="low"),
    ]

    monkeypatch.setattr(
        bootstrap_module,
        "get_event_logs",
        lambda household_id, event_type=None, limit=100: rows if event_type == "email_parsed" else [],
    )
    ...
    assert any(item.title == "Email: School reminder" for item in email_notifications)
    assert any(item.notification_id == "notif:email_summary:rollup_low" for item in email_notifications)
